...
fn test_store_insert_get() {
    ...
    store.insert("key".to_string(), 42, PathBuf::from("test"), UnloadPolicy::default());
...
fn test_store_remove() {
    let store = Store::<i32>::new();
    store.insert("key".to_string(), 42, PathBuf::from("test"), UnloadPolicy::Removable);
...
fn test_store_persistent() {
    let store = Store::<i32>::new();
    store.insert("key".to_string(), 42, PathBuf::from("test"), UnloadPolicy::Persistent);
